...
class TFL:
    ...
    def get_cord_tfl(self, current_frame: FrameContainer, image_path: str) -> (list, list, np.array):
    ...
    def is_tfl(self, candidate: np.array, auxiliary: list, image_path: str) -> (list, list):
        ...
        for i, coord in enumerate(candidate):
            croped_images = self.crop_image(image_pad, coord)
            result = network(croped_images, model)
    ...
    def detect_of_source_lights(self, image_path: str) ->(np.array, list):
