...
def load_data(dir_set: str) -> dict:
    ...
    url_image = "E:\\אסתי\\תכנות\\פרויקטים\\mobilye_project\\מעודכן\\Mobileye-project\\phase_II\\data\\leftImg8bit\\"
    url_label = "E:\\אסתי\\תכנות\\פרויקטים\\mobilye_project\\מעודכן\\Mobileye-project\\phase_II\\data\\gtFine\\"

    return {'images': load_tfl_data(url_image + dir_set, image_suffix),
            'labels': load_tfl_data(url_label + dir_set, label_suffix)}


def get_random_pixel(pixels: List[int]) -> (int, int):
...
def call_write_to_bin_file(x_coords: list, y_coords: list, label_image: np.ndarray, padded_image: np.ndarray, data,
                           labels, data_name: str) -> None:
    count = 0
...
def write_label_to_binary_file(file_name, label: int, data_name: str):
