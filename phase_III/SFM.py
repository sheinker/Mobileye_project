...
def find_corresponding_points(p: np.ndarray, norm_pts_rot: np.ndarray, foe: np.ndarray) -> (int, np.ndarray):
    x, y = 0, 1
...
def calc_dist(p_curr: np.ndarray, p_rot: np.ndarray, foe: np.ndarray, tZ: float) -> int:
